        ...
        if Pug_Runner in ctx.author.roles:
                ...
                if len(list_of_supports) < 4:
                    for member in member_list:
                        if DPS or Main_DPS or Tank or Main_Tank in member.roles:
                            list_of_supports_fillers = list_of_supports + [member]
                    list_of_supports = list_of_supports + list_of_supports_fillers
                    while list_of_supports > 4:
                        choice = random.choice(list_of_supports)
                        if Support or Main_Support not in choice.roles:
                            choice.remove(choice)

                if len(list_of_supports) == 4:
                    choice1 = random.choice(list_of_supports)
                    team_1_supports = team_1_supports + [choice1.mention]
                    await choice1.send("You're playing **SUPPORT** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    ...
                    await choice2.send("You're playing **SUPPORT** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    ...
                    await choice3.send("You're playing **SUPPORT** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    ...
                    await choice4.send("You're playing **SUPPORT** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            ...
            if len(list_of_tanks) < 4:  # what to do if there are less then 4 maintanks
                ...
                if len(list_of_tanks) < 4:
                    for member in member_list:
                        if DPS or Main_DPS or Support or Main_Support in member.roles:
                            list_of_tank_fillers = list_of_tanks + [member]
                    list_of_tanks = list_of_tanks + list_of_tank_fillers
                    while list_of_tanks > 4:
                        choice = random.choice(list_of_tanks)
                        if Tank or Main_Tank not in choice.roles:
                            choice.remove(choice)

                if len(list_of_tanks) == 4:
                    choice1 = random.choice(list_of_tanks)
                    team_1_tanks = team_1_tanks + [choice1.mention]
                    await choice1.send("You're playing **TANK** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice1.move_to(team_1_vc)
                    ...
                    await choice2.send("You're playing **TANK** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice2.move_to(team_1_vc)
                    ...
                    await choice3.send("You're playing **TANK** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice3.move_to(team_2_vc)
                    ...
                    await choice4.send("You're playing **TANK** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice4.move_to(team_2_vc)
            ...
            if len(list_of_tanks) == 4:
                ...
                choice3 = random.choice(list_of_tanks)
                team_2_tanks = team_2_tanks + [choice3.mention]
                await choice3.send("You're playing **TANK** for Team 2, at the PUG that took place in: " + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                await choice3.move_to(team_2_vc)
                list_of_tanks.remove(choice3)
                list_of_members_tank.remove(choice3)
            ...
            if len(list_of_dps) < 4:  # what to do if there are less then 4 maintanks
                ...
                if len(list_of_dps) < 4:
                    ...
                    list_of_members_dps = list_of_dps

                if len(list_of_dps) == 4:
                    choice1 = random.choice(list_of_dps)
                    team_1_dps = team_1_dps + [choice1.mention]
                    await choice1.send("You're playing **DPS** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice1.move_to(team_1_vc)
                    list_of_dps.remove(choice1)

                    choice2 = random.choice(list_of_dps)
                    team_1_dps = team_1_dps + [choice2.mention]
                    await choice2.send("You're playing **DPS** for Team 1, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice2.move_to(team_1_vc)
                    list_of_dps.remove(choice2)

                    choice3 = random.choice(list_of_dps)
                    team_2_dps = team_2_dps + [choice3.mention]
                    await choice3.send("You're playing **DPS** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice3.move_to(team_2_vc)
                    list_of_dps.remove(choice3)

                    choice4 = random.choice(list_of_dps)
                    team_2_dps = team_2_dps + [choice4.mention]
                    await choice4.send("You're playing **DPS** for Team 2, at the PUG that took place in: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await choice4.move_to(team_2_vc)
